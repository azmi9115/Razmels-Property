...
// Replace the old glassmorphism classes globally in this file
content = content.replace(/border-slate-200\/60 shadow-sm bg-white\/70 backdrop-blur-xl/g, 'bg-white rounded-2xl border border-slate-200 shadow-sm hover:shadow-md transition-shadow duration-300');

// Inject the insights logic right after `const finalBalance = totalIncome - totalExpense`
const replaceLogic = '  const finalBalance = totalIncome - totalExpense';
const logicInjection = `  const finalBalance = totalIncome - totalExpense
...
content = content.replace(replaceLogic, logicInjection);

// Inject the UI card right after the summary cards end
const replaceUI = `        </Card>
      </div>
...
content = content.replace(replaceUI, uiInjection);

// Also make the title of Buku Besar match the serif elegant look
const replaceHeader = `<CardTitle className="text-lg flex items-center gap-2">
                <Wallet className="h-5 w-5 text-primary" />
                Buku Besar
              </CardTitle>
              <CardDescription>Catatan semua transaksi keluar masuk</CardDescription>`;
              
const newHeader = `<CardTitle className="text-xl font-bold text-slate-800 flex items-center gap-2">
                <Wallet className="h-5 w-5 text-slate-700" />
                Buku Besar
              </CardTitle>
              <CardDescription className="text-sm font-medium text-slate-500 mt-1">Catatan semua transaksi keluar masuk</CardDescription>`;
              
content = content.replace(replaceHeader, newHeader);

fs.writeFileSync(path, content, 'utf8');
console.log('Successfully injected insights into cashflow page!');
